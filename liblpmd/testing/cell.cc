...
class lpmd::CellImpl
{
 public:
  ...
  CellImpl(double a, double b, double c, double alpha, double beta, double gamma)
  {
   ...
   v[2][1] = c*tmp;
   v[2][2] = c*sqrt(sin(beta)*sin(beta)-tmp*tmp);
   for (int i=0;i<3;++i) p[i] = true;
   UpdateTransfMatrix();
  }
 ...
 void ConvertToInternal(Vector & v) const
 {
  if (nonortg > 1.0e-10)
  {
   ...
   const double y = v[1];
   const double z = v[2];
   v[0] = x*tm[0][0]+y*tm[1][0]+z*tm[2][0];
   v[1] = x*tm[0][1]+y*tm[1][1]+z*tm[2][1];
   v[2] = x*tm[0][2]+y*tm[1][2]+z*tm[2][2];
  }
 }
 ...
 void ConvertToExternal(Vector & v) const
 {
  if (nonortg > 1.0e-10)
  {
   ...
   const double y = v[1];
   const double z = v[2];
   v[0] = x*itm[0][0]+y*itm[1][0]+z*itm[2][0];
   v[1] = x*itm[0][1]+y*itm[1][1]+z*itm[2][1];
   v[2] = x*itm[0][2]+y*itm[1][2]+z*itm[2][2];
  }
 }
...
{ 
 ...
 impl->v[0][1] = 0.0e0;
 impl->v[0][2] = 0.0e0;
 impl->v[1][0] = b*cos(gamma);
 impl->v[1][1] = b*sin(gamma);
 impl->v[1][2] = 0.0e0;
 impl->v[2][0] = c*cos(beta);
 double tmp=(cos(alpha)-cos(gamma)*cos(beta))/sin(gamma);
 impl->v[2][1] = c*tmp;
 impl->v[2][1] = c*sqrt(sin(beta)*sin(beta)-tmp*tmp);
 UpdateTransfMatrix();
} 
...
{
 Vector tmp = Cross(impl->v[0], impl->v[1]);

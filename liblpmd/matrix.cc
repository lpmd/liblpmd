...
Matrix & Matrix::operator=(const Matrix & m)
{
 ...
 {
  std::cerr << "Assignment operator, equal sizes" << '\n';
  for (long i=0;i<nc;++i) col_labels.push_back(m.GetLabel(i));
  for (long j=0;j<m.Rows();++j)
  {
   for (long i=0;i<m.Columns();++i) Set(i, j, m.Get(i, j));
  }
  std::cerr << "End of assignment operator, equal sizes" << '\n';
 }
 else
 {
  if (values != NULL)
...
Matrix & Matrix::operator+=(const Matrix & m)
{
 ...
 {
  std::cerr << "Matrix sizes differ, Rows: " << Rows() << " != " << m.Rows() << ", Columns: " << Columns() << " != " << m.Columns() << '\n';
...
Matrix lpmd::operator+(const Matrix & a, const Matrix & b)
{
 ...
 {
  std::cerr << "Matrix sizes differ, Rows: " << a.Rows() << " != " << b.Rows() << ", Columns: " << a.Columns() << " != " << b.Columns() << '\n';
  throw MatrixException();
 }
}
